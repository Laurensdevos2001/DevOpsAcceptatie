    ...
    public class PhysicalFileProvider : PhysicalFileProviderBase
    {
        ...
        protected string GetRelativePath(string rootPath, string fullPath)
        {
            if (!String.IsNullOrEmpty(rootPath) && !String.IsNullOrEmpty(fullPath))
            {
                ...
                if (!string.IsNullOrEmpty(this.hostName))
                {
                    int rootIndex = rootPath.IndexOf(this.hostName, StringComparison.CurrentCultureIgnoreCase);
                    if (rootIndex >= 0)
                    {
                        rootPath = rootPath.Substring(rootIndex + this.hostName.Length);
                    }

                    int fullIndex = fullPath.IndexOf(this.hostName, StringComparison.CurrentCultureIgnoreCase);
                    if (fullIndex >= 0)
                    {
                        fullPath = fullPath.Substring(fullIndex + this.hostName.Length);
                    }
                ...
                if (rootDirectory.FullName.EndsWith(Path.DirectorySeparatorChar.ToString()))
                {
                ...
                {
                    int startIndex = fullPath.IndexOf(rootDirectory.FullName + Path.DirectorySeparatorChar);
                    if (startIndex >= 0)
                    {
                        return Path.DirectorySeparatorChar + fullPath.Substring(startIndex + rootDirectory.FullName.Length + 1);
                    }
                }
